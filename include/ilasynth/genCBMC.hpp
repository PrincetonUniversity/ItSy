...
protected:
  ...
  std::vector<std::pair<CVar*, CVar*>>
      _updates; // it seems that _updates is only used for the Update fun
...
protected:
  ...
  void dumpDec(std::ostream& out, const std::string& modelName,
  ...
  void dumpCDec(std::ostream& out, const std::string& modelName,
...
private:
  ...
  template <class T>
  void checkAndInsert(std::map<std::string, T*>& mp, const std::string& name,
                      T* var, bool force = false);
