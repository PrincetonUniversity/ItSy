...
struct SupportVars {
  ...
  struct mem_info_t {
    ...
    bool operator==(const mem_info_t& that) const {
      return mem->equal(that.mem) && addr->equal(that.addr) &&
...
protected:
  ...
  nptr_t _synthesize(const std::string& name, const nptr_t& de_expr,
                     const nptr_t& expr, const z3::expr& y, PyObject* pyfun);

  nptr_t _synthesizeEx(const std::string& name, const nptr_t& de_expr,
                       const nptr_t& expr, const z3::expr& y, PyObject* pyfun);

  z3::expr _createSynMiter(const nptr_t& ex);

  void _initSynthesis();
  nptr_t _synthesizeOp(const std::string& name, const nptr_t& var,
