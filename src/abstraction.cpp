...
int Abstraction::getObjId() { return objCnt++; }

Abstraction::Abstraction(const std::string& n)
    : MAX_SYN_ITER(200), parent(NULL), name(n), fetchExpr(NULL),
      fetchValid(BoolConst::get(true)), paramSyn(1), reduceWhenImport(0),
      _ht(NULL) {}

Abstraction::Abstraction(Abstraction* p, const std::string& n)
    : MAX_SYN_ITER(200), parent(p), name(n), fetchExpr(NULL),
      fetchValid(BoolConst::get(true)), paramSyn(1),
      reduceWhenImport(p->reduceWhenImport), _ht(NULL) {
...
void Abstraction::initMap(nmap_t& from_m, nmap_t& to_m) {
  ...
  for (auto&& fn : from_m) {
    to_m.insert(
        {fn.first,
         npair_t(
             &(fn.second))}); // without & it uses the default copy constructor
  }
}

// ---------------------------------------------------------------------- //
NodeRef* Abstraction::addInp(const std::string& name, int width) {
  if (!checkAndInsertName(INP, name))
    return NULL;
...
NodeRef* Abstraction::addBit(const std::string& name) {
  if (!checkAndInsertName(BIT, name))
    return NULL;
...
NodeRef* Abstraction::addReg(const std::string& name, int w) {
  if (!checkAndInsertName(REG, name))
    return NULL;
...
NodeRef* Abstraction::addMem(const std::string& name, int aw, int dw) {
  if (!checkAndInsertName(MEM, name))
    return NULL;
...
NodeRef* Abstraction::addFun(const std::string& name, int rw,
                             const py::list& l) {
  if (!checkAndInsertName(FUN, name))
    return NULL;
  std::vector<int> argW;
...
nmap_t* Abstraction::getMap(const std::string& name, NodeRef* n) {
  ...
  if (n->node->type.isBool()) {
    m = &bits;
  } else if (n->node->type.isBitvector()) {
    m = &regs;
  } else if (n->node->type.isMem()) {
    m = &mems;
  } else {
...
nmap_t::const_iterator
Abstraction::findInMapNoExcept(const std::string& name) const {
  // try to find in each of the maps.
...
NodeRef* Abstraction::getInit(const std::string& name) const {
  auto pos =
      findInMap(name); // init can never be NULL based on the constructors
  ILA_ASSERT(pos->second.init.get() != NULL,
             "init function of " + name + " is not set.");
...
void Abstraction::setIpred(const std::string& name, NodeRef* n) {
  ...
  // check types.
  if (!n->node->type.isBool()) {
    throw PyILAException(PyExc_TypeError, "Ipred expression must be boolean.");
  }
...
NodeRef* Abstraction::getNext(const std::string& name) const {
  ...
  if (!pos->second.next.get())
    throw PyILAException(PyExc_RuntimeError,
                         "Next expression has not been set for variable:" +
                             name);
...
// ---------------------------------------------------------------------- //
AbstractionWrapper* Abstraction::addUAbs(const std::string& name,
                                         NodeRef* validN) {
  ...
  if (uabs.find(name) != uabs.end()) {
    throw PyILAException(PyExc_RuntimeError, "Abstraction already exists.");
    return NULL;
  }
...
void Abstraction::connectUInst(const std::string& name,
                               const abstraction_ptr_t& uab) {
  ...
  if (!nxt.get()) // this can happen when someone forgets to set "next" first
    throw PyILAException(PyExc_RuntimeError, "Please specify next expr for:" +
                                                 name + " before connecting.");

  if (nxt->type != nxt_u->type) {
    throw PyILAException(PyExc_TypeError, "Types don't match for: " + name);
    return;
  }
...
NodeRef* Abstraction::getFetchExpr() const { return new NodeRef(fetchExpr); }
...
NodeRef* Abstraction::getFetchValid() const { return new NodeRef(fetchValid); }
nptr_t Abstraction::getFetchValidNode() const { return fetchValid; }
...
const nptr_vec_t& Abstraction::getDecodeNodes() const { return decodeExprs; }

// ---------------------------------------------------------------------- //
void Abstraction::addAssumption(NodeRef* expr) {
  if (!expr->node->type.isBool()) {
    throw PyILAException(PyExc_TypeError, "Assumption must be a boolean.");
    return;
  }
...
void Abstraction::synthesizeAll(PyObject* pyfun) {
  if (!doesNextExist(regs))
    return;
  if (!doesNextExist(bits))
    return;
  if (!doesNextExist(mems))
    return;
...
void Abstraction::synthesizeReg(const std::string& name, PyObject* pyfun) {
  ...
  if (pos == bits.end()) {
    throw PyILAException(PyExc_RuntimeError, "Unable to find name: " + name);
    return;
  }
...
// ---------------------------------------------------------------------- //
NodeRef* Abstraction::synthesizeElement(const std::string& name, NodeRef* ex,
                                        PyObject* pyfun) {
...
void Abstraction::extractModelValues(Z3ExprAdapter& c, z3::model& m,
                                     py::dict& d) {
  using namespace z3;
  using namespace py;
...
bool Abstraction::areEqual(NodeRef* left, NodeRef* right) const {
  ...
  if (left->node->type != right->node->type) {
    throw PyILAException(PyExc_TypeError, "Types do not match.");
    return false;
  }
...
bool Abstraction::areEqualAssump(NodeRef* assump, NodeRef* left,
                                 NodeRef* right) {
  using namespace z3;
...
bool Abstraction::EQcheckSimple(Abstraction* a1, Abstraction* a2) {
  ...
  for (auto&& reg : a1->regs) // just iterate on one side
  {
    ...
    if (DetermineUnrollBound(a1, name) == 1 &&
        DetermineUnrollBound(a2, name) == 1) {
...
bool Abstraction::bmc(unsigned n, Abstraction* abs, NodeRef* r, bool init,
                      NodeRef* initAssump) {
  using namespace z3;
...
bool Abstraction::bmc(unsigned n1, Abstraction* a1, NodeRef* r1, unsigned n2,
...
void Abstraction::exportListToFile(const py::list& l,
                                   const std::string& fileName) const {
  if (py::len(l) < 2) {
    throw PyILAException(PyExc_RuntimeError, "Must have at least two asts.");
    return;
  }
...
void Abstraction::generateVerilogToFile(const std::string& fileName,
...
void Abstraction::exportAllToStream(std::ofstream& out) const {
  ...
  // inputs.
  out << ".inps: ";
  for (nmap_t::const_iterator it = inps.begin(); it != inps.end(); it++) {
    out << " " << it->first << " ";
  ...
  // registers.
  out << ".regs: ";
  for (nmap_t::const_iterator it = regs.begin(); it != regs.end(); it++) {
    out << "\n" << it->first << " ";
    expt.exportAst(out, it->second.var.get());
    out << " ";
  ...
  // bits.
  out << ".bits: ";
  for (nmap_t::const_iterator it = bits.begin(); it != bits.end(); it++) {
    out << "\n" << it->first << " ";
    expt.exportAst(out, it->second.var.get());
    out << " ";
    expt.exportAst(out, it->second.init.get());
  ...
  // mems.
  out << ".mems: ";
  for (nmap_t::const_iterator it = mems.begin(); it != mems.end(); it++) {
    out << "\n" << it->first << " ";
    expt.exportAst(out, it->second.var.get());
    out << " ";
    expt.exportAst(out, it->second.init.get());
  ...
  // funs.
  out << ".funs: ";
  for (nmap_t::const_iterator it = funs.begin(); it != funs.end(); it++) {
    out << "\n" << it->first << " ";
    expt.exportAst(out, it->second.var.get());
  }
  out << "\n.funs_end\n";
  ...
  // uabs name & valid
  out << ".uabs:";
  for (uabs_map_t::const_iterator it = uabs.begin(); it != uabs.end(); it++) {
    out << "\n" << it->first << " ";
...
void Abstraction::importAllFromStream(
    std::ifstream& in,
    bool Clear = true) // Clear is unset when called by parent ILA
{
  // Clear all variables.
  names.clear();
  inps.clear();
...
// ---------------------------------------------------------------------- //
nptr_t Abstraction::_synthesize(const std::string& name,
                                const nptr_vec_t& all_assumps, const nptr_t& ex,
                                PyObject* pyfun) {
  using namespace py;
  using namespace z3;
  ...
  log2("Abstraction._synthesize")
      << "finished after " << i << " SMT calls." << std::endl;
...
CVerifGen* Abstraction::generateCBMCC(bool hier) const {
  ...
  for (unsigned i = 0; i < assumps.size(); i++) {
    CFun* assFun =
        gen->addFun("assumps_" + boost::lexical_cast<std::string>(i));
...
void Abstraction::setUpdateToFunction(CppSimGen* gen, CppFun* fun, nptr_t valid,
                                      bool doHier) const {
