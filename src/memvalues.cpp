...
// ---------------------------------------------------------------------- //
MemValues::MemValues() : MAX_ADDR(0), def_value(0) {}

MemValues::MemValues(int aw, int dw, const py::object& dv)
    : type(NodeType::getMem(aw, dw)), MAX_ADDR(mp_int_t(1) << aw),
      def_value(to_cpp_int(dv)) {}

MemValues::MemValues(int aw, int dw, const map_t values,
                     const mp_int_t def_value)
    : type(NodeType::getMem(aw, dw)), MAX_ADDR(mp_int_t(1) << aw),
      def_value(def_value), values(values) {}

MemValues::MemValues(Z3ExprAdapter& c, const z3::model& m, const MemVar* mem)
    : type(mem->type), MAX_ADDR(mp_int_t(1) << type.addrWidth) {
  using namespace z3;
  ...
  if (Z3_get_decl_kind(c.ctx(), mfd) == Z3_OP_CONST_ARRAY) {
    ...
    ILA_ASSERT(Z3_get_decl_num_parameters(c.ctx(), mfd) == 1,
               "BUG unable to handle");
    Z3_sort memsort = Z3_get_decl_sort_parameter(c.ctx(), mfd, 0);
    ILA_ASSERT(Z3_get_sort_kind(c.ctx(), memsort) == Z3_ARRAY_SORT,
               "expect ARRAY sort.");
    ...
    ILA_ASSERT(Z3_get_decl_kind(c.ctx(), mfd) == Z3_OP_AS_ARRAY,
               "Expected array op.");
    ILA_ASSERT(Z3_is_app(c.ctx(), mval), "Expected application.");
    ILA_ASSERT(Z3_get_decl_num_parameters(c.ctx(), mfd) == 1,
               "Expected one parameter.");
    ILA_ASSERT(Z3_get_decl_parameter_kind(c.ctx(), mfd, 0) ==
                   Z3_PARAMETER_FUNC_DECL,
               "Expected parameter to a function declaration.");

    // get the function interpretation (also SO).
    func_decl ffd =
        func_decl(c.ctx(), Z3_get_decl_func_decl_parameter(c.ctx(), mfd, 0));
...
MemValues::MemValues(const MemValues& that)
    : type(that.type), MAX_ADDR(that.MAX_ADDR), def_value(that.def_value),
      values(that.values) {}
...
// ---------------------------------------------------------------------- //
py::object MemValues::getDefault() const { return to_pyint(def_value); }
