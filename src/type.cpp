...
std::ostream& operator<<(std::ostream& out, NodeType const& ntype) {
  ...
  case NodeType::BITVECTOR:
    ...
    break;
  case NodeType::MEM:
    out << "(type mem " << ntype.addrWidth << " " << ntype.dataWidth << ")";
    break;
  case NodeType::FUNC:
...
NodeType::NodeType(Type t)
    : type(BOOL), bitWidth(0), addrWidth(0), dataWidth(0) {
  if (t != BOOL) {
...
NodeType::NodeType(Type t, int w)
    : type(BITVECTOR), bitWidth(w), addrWidth(0), dataWidth(0) {
  if (t != BITVECTOR) {
    throw PyILAException(PyExc_TypeError,
                         "Invalid type argument in constructor for Type.");
  }
...
NodeType::NodeType(Type t, int aw, int dw)
    : type(MEM), bitWidth(0), addrWidth(aw), dataWidth(dw) {
  if (t != MEM) {
    throw PyILAException(PyExc_TypeError,
                         "Invalid type argument in constructor for Type.");
  }
...
NodeType::NodeType(Type t, int rw, const std::vector<int>& aw)
    : type(FUNC), bitWidth(rw), addrWidth(0), dataWidth(0), argsWidth(aw) {
  if (t != FUNC) {
    throw PyILAException(PyExc_TypeError,
                         "Invalid type argument in constructor for Type.");
  }
...
bool NodeType::operator==(const NodeType& t) const {
  if (type == BOOL && t.type == BOOL) {
    ...
    return addrWidth == t.addrWidth && dataWidth == t.dataWidth;
  } else if (type == FUNC && t.type == FUNC) {
    if ((bitWidth != t.bitWidth) || (argsWidth.size() != t.argsWidth.size())) {
      return false;
    } else {
