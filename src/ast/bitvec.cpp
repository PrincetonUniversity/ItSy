...
// ---------------------------------------------------------------------- //
BitvectorExpr::BitvectorExpr(int width) : Node(NodeType::getBitvector(width)) {}

BitvectorExpr::BitvectorExpr(NodeType t) : Node(t) {
...
bool BitvectorVar::equal(const Node* that_) const {
  ...
  if (that != NULL) {
    return that->type == this->type && that->name == this->name;
  } else {
    return false;
  }
}
...
// ---------------------------------------------------------------------- //
unsigned BitvectorOp::nArgs() const { return args.size(); }

unsigned BitvectorOp::nParams() const { return params.size(); }
...
int BitvectorOp::getBinaryResultWidth(Op op, const nptr_t& n1,
                                      const nptr_t& n2) {
  // FIXME: add more code when operators are added.
  if (op >= ADD && op <= MUL) {
...
int BitvectorOp::getBinaryResultWidth(Op op, const nptr_t& n1, const nptr_t& n2,
                                      int param) {
  if (op == READMEMBLOCK) {
...
int BitvectorOp::getBinaryResultWidth(Op op, const nptr_t& n1, int param) {
...
int BitvectorOp::getNaryResultWidth(Op op, nptr_vec_t& args) {
  // FIXME: add more code when operators are added.
...
int BitvectorOp::checkBinaryOpWidth(Op op, const nptr_t& n1, const nptr_t& n2,
                                    int width) {
  // FIXME: add more code when operators are added.
  if (op >= ADD && op <= MUL) {
...
int BitvectorOp::checkBinaryOpWidth(Op op, const nptr_t& n1, int param,
                                    int width) {
...
int BitvectorOp::checkNaryOpWidth(Op op, nptr_vec_t& args, int width) {
...
int BitvectorOp::checkNaryOpWidth(Op op, nptr_vec_t& args,
                                  std::vector<int>& params, int width) {
  // FIXME: modify the code if other n-ary ops are added
...
BitvectorOp::BitvectorOp(Op op, const nptr_t& n1)
    : BitvectorExpr(getUnaryResultWidth(op, n1)), arity(UNARY), op(op) {

  // check if for the correct operator.
  if (!isUnary(op)) {
    throw PyILAException(PyExc_ValueError,
                         "Invalid unary operator: " + operatorNames[op]);
  }
...
BitvectorOp::BitvectorOp(Op op, const nptr_t& n1, int param)
    : BitvectorExpr(getBinaryResultWidth(op, n1, param)), arity(UNARY), op(op) {
  if (!isUnary(op)) {
    throw PyILAException(PyExc_ValueError,
                         "Invalid binary operator: " + operatorNames[op]);
  }
  if (!checkUnaryOpWidth(op, n1, type.bitWidth)) {
    throw PyILAException(PyExc_TypeError,
...
// constructor: extract
BitvectorOp::BitvectorOp(Op op, const nptr_t& n1, int p1, int p2)
    : BitvectorExpr((p1 - p2) + 1), arity(UNARY), op(op) {
  if (op != EXTRACT) {
    throw PyILAException(PyExc_ValueError,
                         "Invalid operator: " + operatorNames[op]);
  } else if (p1 < p2) {
    throw PyILAException(PyExc_ValueError, "Invalid indices to extract.");
  }
  if (!(n1->type.isBitvector() && n1->type.bitWidth >= p1 &&
        n1->type.bitWidth >= p2)) {
    throw PyILAException(PyExc_TypeError,
...
BitvectorOp::BitvectorOp(Op op, const nptr_t& n1, const nptr_t& n2)
    : BitvectorExpr(getBinaryResultWidth(op, n1, n2)), arity(BINARY), op(op) {
  if (!isBinary(op)) {
    throw PyILAException(PyExc_ValueError,
                         "Invalid binary operator: " + operatorNames[op]);
  }
...
BitvectorOp::BitvectorOp(Op op, const nptr_t& n1, const nptr_t& n2, int blocks,
                         endianness_t e)
    : BitvectorExpr(getBinaryResultWidth(op, n1, n2, blocks)), arity(BINARY),
      op(op) {
  if (op != READMEMBLOCK) {
    throw PyILAException(PyExc_ValueError,
  ...
  if (blocks < 1) {
    throw PyILAException(PyExc_ValueError, "Invalid number of chunks.");
  }
  if (e != LITTLE_E && e != BIG_E) {
    throw PyILAException(PyExc_ValueError, "Invalid endianness value.");
  }
...
BitvectorOp::BitvectorOp(Op op, nptr_vec_t& args_)
    : BitvectorExpr(getNaryResultWidth(op, args_)), arity(NARY), op(op),
      args(args_) {
...
BitvectorOp::BitvectorOp(const BitvectorOp* other, nptr_vec_t& args_)
    : BitvectorExpr(other->type.bitWidth), arity(other->arity), op(other->op),
      args(args_), params(other->params) {}
...
Node* BitvectorOp::clone() const {
  if (arity == UNARY) {
    ...
    ILA_ASSERT(args.size() >= 1, "Nary op must have more than one arguments.");
    nptr_vec_t newargs;
