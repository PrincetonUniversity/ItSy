...
std::size_t hash_value(const NodeType& ntype) { return ntype.hash_val; }
...
std::size_t hash_value(const MemValues& mv) {
  ...
  boost::hash_combine(seed, boost::lexical_cast<std::string>(mv.MAX_ADDR));
  boost::hash_combine(seed, boost::lexical_cast<std::string>(mv.def_value));
  for (auto it : mv.values) {
    boost::hash_combine(seed, boost::lexical_cast<std::string>(it.first));
    boost::hash_combine(seed, boost::lexical_cast<std::string>(it.second));
  }
  return seed;
}
} // namespace ila
