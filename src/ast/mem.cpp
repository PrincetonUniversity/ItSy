...
const std::string MemOp::operatorNames[] = {"invalid", "store", "storeblock",
                                            "ite"};
// ---------------------------------------------------------------------- //
MemExpr::MemExpr(int aw, int dw) : Node(NodeType::getMem(aw, dw)) {}

MemExpr::MemExpr(NodeType t) : Node(t) {
...
// ---------------------------------------------------------------------- //
MemVar::MemVar(const std::string& n, int aw, int dw) : MemExpr(aw, dw) {
  this->name = n;
}
...
bool MemVar::equal(const Node* that_) const {
  ...
  if (that) {
    return that->name == name && that->type == type;
  } else {
    return false;
  }
}

std::ostream& MemVar::write(std::ostream& out) const { return (out << name); }

// ---------------------------------------------------------------------- //
MemConst::MemConst(const MemValues& mv)
    : MemExpr(mv.type.addrWidth, mv.type.dataWidth), memvalues(mv) {}
...
// ---------------------------------------------------------------------- //
Node* MemConst::clone() const { return new MemConst(*this); }
...
MemOp::MemOp(Op o, const nptr_t& a0, const nptr_t& a1, const nptr_t& a2)
    : MemExpr(o == MemOp::STORE ? a0->type : a1->type), op(o),
      endian(UNKNOWN_E) {
  ILA_ASSERT(op == MemOp::STORE || op == MemOp::ITE, "Invalid op value.");

  if (op == MemOp::STORE) {
    if (!a0->type.isMem()) {
      throw PyILAException(PyExc_TypeError, "Memory type error.");
    } else if (!a1->type.isBitvector(a0->type.addrWidth)) {
      throw PyILAException(PyExc_TypeError, "Address type error.");
    ...
    if (!a0->type.isBool()) {
      throw PyILAException(PyExc_TypeError, "Condition must be a boolean.");
...
MemOp::MemOp(Op op, const nptr_t& mem, const nptr_t& addr, const nptr_t& data,
             endianness_t e)
    : MemExpr(mem->type.addrWidth, mem->type.dataWidth), op(STOREBLOCK),
      endian(e) {
  ILA_ASSERT(op == STOREBLOCK, "Invalid op.");
  // check memory type //
  if (!mem->type.isMem()) {
    throw PyILAException(PyExc_TypeError, "First argument must be a memory.");
    return;
  }
  // check address type and width //
  if (!addr->type.isBitvector(mem->type.addrWidth)) {
    throw PyILAException(PyExc_TypeError, "Invalid address type.");
    return;
  }
  // check data type and width //
  if (!data->type.isBitvector() ||
      data->type.bitWidth % mem->type.dataWidth != 0) {
    throw PyILAException(PyExc_TypeError, "Invalid data type.");
    return;
  }
...
MemOp::MemOp(const MemOp& that)
    : MemExpr(that.type.addrWidth, that.type.dataWidth), op(that.op),
      endian(that.endian), args(that.args) {
  ILA_ASSERT(args.size() == 3, "Mem op size mismatch.");
  ILA_ASSERT(op != STOREBLOCK || endian != UNKNOWN_E, "endian undefined!");
}
...
// ---------------------------------------------------------------------- //
Node* MemOp::clone() const { return new MemOp(*this); }
...
bool MemOp::equal(const Node* that_) const {
  ...
  if (that) {
    ...
    for (unsigned i = 0; i != args.size(); i++) {
      if (!args[i]->equal(that->args[i].get()))
        return false;
    }
    return true;
  } else {
...
std::ostream& MemOp::write(std::ostream& out) const {
  out << "(" << operatorNames[op] << " " << *args[0].get() << " "
      << *args[1].get() << " " << *args[2].get();

  if (endian != UNKNOWN_E) {
    out << " #" << (endian == LITTLE_E ? "little-endian" : "big-endian");
  }
...
// ---------------------------------------------------------------------- //
unsigned MemOp::nArgs() const { return args.size(); }
