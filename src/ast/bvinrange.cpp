...
BVInRange::BVInRange(const BVInRange& that)
    : BitvectorExpr(that.type.bitWidth), lo(that.lo), hi(that.hi) {}

BVInRange::~BVInRange() {}

Node* BVInRange::clone() const { return new BVInRange(*this); }

bool BVInRange::equal(const Node* that_) const {
  auto that = dynamic_cast<const BVInRange*>(that_);
  if (that) {
    return that->type == type && that->lo->equal(lo.get()) &&
...
unsigned BVInRange::nArgs() const { return 2; }
