...
void ImExport::exportAst(std::ostream& out, const Node* n) {
  ...
  if ((boolvar = dynamic_cast<const BoolVar*>(n))) {
    ...
    out << "( bvVar " << bvvar->getName() << " " << bvvar->type.bitWidth
        << " )";
  } else if ((bvconst = dynamic_cast<const BitvectorConst*>(n))) {
    out << "( bvConst " << bvconst->getName() << " " << bvconst->type.bitWidth
        << " " << bvconst->vstr() << " )";
    ...
    out << "( memConst " << memconst->getName() << " "
        << memconst->type.addrWidth << " " << memconst->type.dataWidth << " ";
    ...
    out << "( funcVar " << funcvar->getName() << " " << funcvar->type.bitWidth
        << " <- (";
...
nptr_t ImExport::importAst(Abstraction* c, std::istream& in) {
  ...
  if (nodeType == "boolVar") {
    ...
    } else if (op >= BoolOp::Op::AND && op <= BoolOp::Op::DISTINCT) {
      nptr_t arg0 = importAst(c, in);
    ...
    ILA_ASSERT(nextChar(in) == ')', "Miss )");
    if (reduceWhenImport == 1)
      FuncReductor->InsertOrDupNoRecursive(nptr);
    return nptr;
    ...
    if (op >= BitvectorOp::Op::NEGATE && op <= BitvectorOp::Op::COMPLEMENT) {
      nptr_t arg0 = importAst(c, in);
      if (nptr == NULL) {
        nptr = nptr_t(new BitvectorOp(op, arg0));
      }
    } else if (op >= BitvectorOp::Op::LROTATE && op <= BitvectorOp::Op::S_EXT) {
      nptr_t arg0 = importAst(c, in);
      int par0 = eatIdx(in);
      if (nptr == NULL) {
    ...
    } else if (op >= BitvectorOp::Op::ADD && op <= BitvectorOp::Op::READMEM) {
      nptr_t arg0 = importAst(c, in);
      nptr_t arg1 = importAst(c, in);
      if (nptr == NULL) {
...
MemOp::Op ImExport::getMemOpType(const std::string& opName) const {
  for (int op = (int)MemOp::INVALID; op <= (int)MemOp::ITE; op++) {
...
BitvectorOp::Op ImExport::getBvOpType(const std::string& opName) const {
  ...
  for (int op = BitvectorOp::Op::INVALID; op <= BitvectorOp::Op::APPLY_FUNC;
       op++) {
...
BoolOp::Op ImExport::getBoolOpType(const std::string& opName) const {
  for (int op = BoolOp::Op::INVALID; op <= BoolOp::Op::IF; op++) {
