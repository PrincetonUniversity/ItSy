...
void MicroUnroller::addInputNodes() {
  ...
  for (auto inp : m_pUAbstraction->getInps()) {
    ILA_ASSERT(inp.second.parent, "Cannot translate micro-inst with standalone "
                                  "input to the SMT template.");
  }
}

void MicroUnroller::_initVar(Z3ExprAdapter& z3expr, const npair_t& p,
                             bool allowInit) {
  ...
  if (!allowInit || var->equal(init)) {
    ...
    log2("Unroller") << "ADD " << c << " to " << nFrame << "," << idx
                     << std::endl;
  } else {
...
void MicroUnroller::addAllStatesToSecondFrame(bool initCondSet) {
  ...
  z3expr_micro.setNameSuffix("_in_" + m_pAbstraction->getName() + "_" +
                             m_pUAbstraction->getName());
  ...
  for (auto b : m_pUAbstraction->getBits())
    if (b.second.parent) {
      ...
      log2("Unroller") << "ADD " << c << " to " << frame() << "," << idx
                       << std::endl;
    } else {
...
void MicroUnroller::addAllStatesToLaterFrame() {
  ...
  for (auto b : m_pUAbstraction->getBits()) {
    if (b.second.next.get()) {
      z3::expr eu =
          z3expr.getExpr(b.second.next.get()); // micro arch will update that
      ...
      ILA_ASSERT(idx == m_mStateIndices[b.second.var.get()],
                 "Put to a wrong place in the frame");
    } else {
      z3::expr e =
          z3expr.getExpr(b.second.var.get()); // its next expression is itself
  ...
  for (auto r : m_pUAbstraction->getRegs()) {
    if (r.second.next.get()) {
      z3::expr eu =
          z3expr.getExpr(r.second.next.get()); // micro arch will update that
      z3::expr ea = z3expr.getExpr(r.second.var.get());
      z3::expr next = z3::ite(valid, eu, ea);
      unsigned idx = addOutput(next.substitute(sub_src, sub_dst).simplify());
      ILA_ASSERT(idx == m_mStateIndices[r.second.var.get()],
                 "Put to a wrong place in the frame");
    } else {
      z3::expr e =
          z3expr.getExpr(r.second.var.get()); // its next expression is itself
      unsigned idx = addOutput(e.substitute(sub_src, sub_dst).simplify());
      ILA_ASSERT(idx == m_mStateIndices[r.second.var.get()],
                 "Put to a wrong place in the frame");
    }
  ...
  for (auto m : m_pUAbstraction->getMems()) {
    if (m.second.next.get()) {
      z3::expr eu =
          z3expr.getExpr(m.second.next.get()); // micro arch will update that
      z3::expr ea = z3expr.getExpr(m.second.var.get());
      z3::expr next = z3::ite(valid, eu, ea);
      unsigned idx = addOutput(next.substitute(sub_src, sub_dst).simplify());
      ILA_ASSERT(idx == m_mStateIndices[m.second.var.get()],
                 "Put to a wrong place in the frame");
    } else {
      z3::expr e =
          z3expr.getExpr(m.second.var.get()); // its next expression is itself
      unsigned idx = addOutput(e.substitute(sub_src, sub_dst).simplify());
      ILA_ASSERT(idx == m_mStateIndices[m.second.var.get()],
                 "Put to a wrong place in the frame");
    }
...
void MicroUnroller::dumpAssertion(NodeRef* assertion,
  ...
  if (m_pSolver->check() != z3::sat) {
    std::cerr << "Assumptions do not hold, unable to check assertion."
              << std::endl;
    m_pSolver->reset();
    // m_pSolver->pop();
    return; // or true ?
...
void MicroUnroller::InsertAssumptSub(z3::expr_vector& src,
                                     z3::expr_vector& dest) {
  for (z3::expr& a : m_Assumps) {
    log2("Unroller") << "Insert assumpt sub before:" << a << std::endl;
    log2("Unroller") << "Insert assumpt sub after:" << a.substitute(src, dest)
                     << std::endl;
...
MicroUnroller* MicroUnroller::NewUnroller(AbstractionWrapper* Abs,
                                          AbstractionWrapper* Uabs,
                                          bool InitCond) {
  return new MicroUnroller("", *(Abs->abs.get()), *(Uabs->abs.get()),
                           MicroUnrollContext, MicroUnrollSolver, InitCond);
}

void MicroUnroller::EqOffline(const std::string& fileName, MicroUnroller* m1,
...
void MicroUnroller::EqOffline(const std::string& fileName, MicroUnroller* m1,
  ...
  for (boost::python::ssize_t i = 0; i < len; i++) {
    ...
    boost::python::object item =
        boost::python::extract<boost::python::tuple>(elem);
    ...
    if (!getName1.check() || !getName2.check())
      throw PyILAException(PyExc_TypeError,
                           "Frame pair must be a pair of string");
...
bool MicroUnroller::EqCheck(MicroUnroller* m1, MicroUnroller* m2,
...
void MicroUnroller::debugFrame() {
  ...
  for (unsigned fn = 0; fn <= frame(); fn++) {
    for (unsigned slot = 0; slot < m_vOutputs[fn].size(); slot++) {
      log1("Unroller") << "Frame (" << fn << "," << slot
                       << ") : " << m_vOutputs[fn][slot] << std::endl;
    }
  }
  log1("Unroller") << "(!) CURRENT FRAMES: END" << std::endl;
#endif
}
void MicroUnroller::EvalEachFrame(z3::model& m, MicroUnroller* u,
                                  ostream& cexf) {
...
bool MicroUnroller::checkMiter(z3::solver& S, z3::expr& e1, z3::expr& e2,
