...
template <typename T>
typename enable_if<has_to_smt2<T, std::string (T::*)(char const*)>::value,
                   std::string>::type
toSmt(T& t) {
...
toSmt(T& t) {
  ...
  return std::string(
      "T::to_smt2() does not exist. Need a newer version (>= 4.4.2) of Z3.");
}

void SMTExport::exportSMT(std::ofstream& out, const Node* n,
...
bool SMTExport::isStageVar(const std::string& inpName,
