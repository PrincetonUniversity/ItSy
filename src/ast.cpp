...
NodeRef::NodeRef() : node(new Node()) {
  throw PyILAException(
      PyExc_RuntimeError,
      "Cannot create node's directly. Use the context object.");
}

NodeRef::NodeRef(Node* n) : node(n) {}

NodeRef::NodeRef(const nptr_t& ptr) : node(ptr) {}

NodeRef::NodeRef(const NodeRef& nr) : node(nr.node) {}
...
// ---------------------------------------------------------------------- //
std::string NodeRef::getName() const { return node->name; }

NodeType NodeRef::getType() const { return node->type; }
...
NodeRef* NodeRef::addInt(int r) const { return _binOp(BitvectorOp::ADD, r); }

NodeRef* NodeRef::raddInt(int r) const { return _binOpR(BitvectorOp::ADD, r); }
...
NodeRef* NodeRef::subInt(int r) const { return _binOp(BitvectorOp::SUB, r); }

NodeRef* NodeRef::rsubInt(int r) const { return _binOpR(BitvectorOp::SUB, r); }
...
NodeRef* NodeRef::udivInt(int r) const { return _binOp(BitvectorOp::UDIV, r); }
...
// urem //
NodeRef* NodeRef::urem(NodeRef* r) { return _binOp(BitvectorOp::UREM, r); }

NodeRef* NodeRef::uremInt(int r) { return _binOp(BitvectorOp::UREM, r); }

NodeRef* NodeRef::ruremInt(int l) { return _binOpR(BitvectorOp::UREM, l); }
...
NodeRef* NodeRef::shlInt(int r) const { return _binOp(BitvectorOp::SHL, r); }

NodeRef* NodeRef::rshlInt(int r) const { return _binOpR(BitvectorOp::SHL, r); }
...
NodeRef* NodeRef::lshrInt(int r) const { return _binOp(BitvectorOp::LSHR, r); }
...
NodeRef* NodeRef::mulInt(int r) const { return _binOp(BitvectorOp::MUL, r); }

NodeRef* NodeRef::rmulInt(int r) const { return _binOpR(BitvectorOp::MUL, r); }
...
NodeRef* NodeRef::eqInt(int r) const { return _cmpOp(BoolOp::EQUAL, r); }

NodeRef* NodeRef::neqInt(int r) const { return _cmpOp(BoolOp::DISTINCT, r); }

NodeRef* NodeRef::ultInt(int r) const { return _cmpOp(BoolOp::ULT, r); }

NodeRef* NodeRef::ugtInt(int r) const { return _cmpOp(BoolOp::UGT, r); }

NodeRef* NodeRef::uleInt(int r) const { return _cmpOp(BoolOp::ULE, r); }

NodeRef* NodeRef::ugeInt(int r) const { return _cmpOp(BoolOp::UGE, r); }

py::object NodeRef::value() const { return node->getValue(); }
...
NodeRef* NodeRef::load(NodeRef* mem, NodeRef* addr) {
  ...
  if (!mt.isMem() || !at.isBitvector(mt.addrWidth)) {
    throw PyILAException(PyExc_TypeError, "Type error in arguments.");
    return NULL;
  }
  return new NodeRef(
      new BitvectorOp(BitvectorOp::READMEM, mem->node, addr->node));
}
...
NodeRef* NodeRef::loadblock(NodeRef* mem, NodeRef* addr, int chunks) {
  ...
  if (chunks <= 0) {
    throw PyILAException(PyExc_ValueError, "Invalid number of blocks.");
    return NULL;
  }

  return new NodeRef(new BitvectorOp(BitvectorOp::READMEMBLOCK, mem->node,
                                     addr->node, chunks, LITTLE_E));
}
...
NodeRef* NodeRef::store(NodeRef* mem, NodeRef* addr, NodeRef* data) {
  ...
  return new NodeRef(
      new MemOp(MemOp::STORE, mem->node, addr->node, data->node));
}
...
NodeRef* NodeRef::storeblock(NodeRef* mem, NodeRef* addr, NodeRef* data) {
  ...
  if (!mt.isMem() || !at.isBitvector(mt.addrWidth) || !dt.isBitvector() ||
      dt.bitWidth % mt.dataWidth != 0) {
    throw PyILAException(PyExc_TypeError, "Type error in arguments.");
    return NULL;
  }
  return new NodeRef(new MemOp(MemOp::STOREBLOCK, mem->node, addr->node,
                               data->node, LITTLE_E));
}
...
NodeRef* NodeRef::storeblockB(NodeRef* mem, NodeRef* addr, NodeRef* data) {
  ...
  return new NodeRef(
      new MemOp(MemOp::STOREBLOCK, mem->node, addr->node, data->node, BIG_E));
}
...
NodeRef* NodeRef::concatList(const py::list& l) {
  ...
  // number of arguments.
  if (py::len(l) < 2) {
    throw PyILAException(PyExc_RuntimeError, "Must have two arguments.");
    return NULL;
  }
  // must all be nodes.
  for (unsigned i = 0; i != py::len(l); i++) {
    ...
    if (ni.check()) {
      ...
      throw PyILAException(PyExc_TypeError,
                           "Argument to concat must be a node.");
      return NULL;
    }
  }
  ...
  for (unsigned i = 0; i != args.size(); i++) {
    if (!args[i]->type.isBitvector()) {
      throw PyILAException(PyExc_TypeError,
                           "Argument to concat must be a bitvector.");
      return NULL;
    }
  }
  ...
  for (int i = args.size() - 2; i >= 0; i--) {
    ...
    if (i == (int)args.size() - 2) {
      nptr_t arg(new BitvectorOp(BitvectorOp::CONCAT, b1, b2));
      expr = arg;
    } else {
      nptr_t arg(new BitvectorOp(BitvectorOp::CONCAT, b1, expr));
      expr = arg;
    }
  }
...
NodeRef* NodeRef::nonzero(NodeRef* obj) { return obj->neqInt(0); }
...
NodeRef* NodeRef::ite(NodeRef& cond, NodeRef& trueExp, NodeRef& falseExp) {
  return _triOp(BoolOp::IF, BitvectorOp::IF, MemOp::ITE, cond, trueExp,
                falseExp);
}
...
NodeRef* NodeRef::choice3(const std::string& name, NodeRef* e1, NodeRef* e2,
                          NodeRef* e3) {
  nptr_vec_t args = {e1->node, e2->node, e3->node};
  return _choice(name, args);
}

NodeRef* NodeRef::choice4(const std::string& name, NodeRef* e1, NodeRef* e2,
                          NodeRef* e3, NodeRef* e4) {
  nptr_vec_t args = {e1->node, e2->node, e3->node, e4->node};
  return _choice(name, args);
}

NodeRef* NodeRef::choice5(const std::string& name, NodeRef* e1, NodeRef* e2,
                          NodeRef* e3, NodeRef* e4, NodeRef* e5) {
  nptr_vec_t args = {e1->node, e2->node, e3->node, e4->node, e5->node};
  return _choice(name, args);
}

NodeRef* NodeRef::choice6(const std::string& name, NodeRef* e1, NodeRef* e2,
                          NodeRef* e3, NodeRef* e4, NodeRef* e5, NodeRef* e6) {
  nptr_vec_t args = {e1->node, e2->node, e3->node,
                     e4->node, e5->node, e6->node};
  return _choice(name, args);
}
...
NodeRef* NodeRef::choiceL(const std::string& name, const py::list& l) {
  nptr_vec_t args;
  if (py::len(l) < 2) {
    throw PyILAException(PyExc_RuntimeError, "Must have at least two choices.");
    return NULL;
  }
...
NodeRef* NodeRef::inRange(const std::string& name, NodeRef* lo, NodeRef* hi) {
...
NodeRef* NodeRef::readSlice(const std::string& name, NodeRef* bv, int w) {
  if (!bv->node->type.isBitvector() || bv->node->type.bitWidth <= w || w <= 0) {
    throw PyILAException(PyExc_TypeError,
                         "Argument to readslice must be a bitvector of width "
                         "greater than result width.");
    return NULL;
  }
  return new NodeRef(ReadSlice::createReadSlice(name, bv->node, w, 1));
}

NodeRef* NodeRef::readChunk(const std::string& name, NodeRef* bv, int w) {
  if (!bv->node->type.isBitvector() || bv->node->type.bitWidth <= w || w <= 0) {
    throw PyILAException(PyExc_TypeError,
                         "Argument to readchunk must be a bitvector of width "
                         "greater than result width.");
    return NULL;
  }
  ...
  return new NodeRef(ReadSlice::createReadSlice(name, bv->node, w, w));
}

NodeRef* NodeRef::writeSlice(const std::string& name, NodeRef* bv,
                             NodeRef* wr) {
  ...
  return new NodeRef(WriteSlice::createWriteSlice(name, bv->node, wr->node, 1));
}

NodeRef* NodeRef::writeChunk(const std::string& name, NodeRef* bv,
                             NodeRef* wr) {
  if (!bv->node->type.isBitvector() || !wr->node->type.isBitvector() ||
      bv->node->type.bitWidth <= wr->node->type.bitWidth) {
  ...
  return new NodeRef(WriteSlice::createWriteSlice(name, bv->node, wr->node,
                                                  wr->node->type.bitWidth));
}
...
size_t NodeRef::hash() const { return hash_value(*this->node.get()); }

// ---------------------------------------------------------------------- //
NodeRef* NodeRef::_unOp(BoolOp::Op opBool, BitvectorOp::Op opBv,
...
NodeRef* NodeRef::_binOp(BoolOp::Op boolOp, BitvectorOp::Op bvOp,
                         const char* opName, NodeRef* other) const {
  if (boolOp != BoolOp::INVALID && node->type.isBool()) {
    return new NodeRef(new BoolOp(boolOp, node, other->node));
  } else if (bvOp != BitvectorOp::INVALID && node->type.isBitvector()) {
    return new NodeRef(new BitvectorOp(bvOp, node, other->node));
  } else {
    throw PyILAException(PyExc_TypeError,
                         std::string("Incorrect type for ") + opName);
  }
...
NodeRef* NodeRef::_binOp(BitvectorOp::Op op, NodeRef* other) const {
  if (node->type.isBitvector()) {
    return new NodeRef(new BitvectorOp(op, node, other->node));
  } else if (node->type.isMem() && other->node->type.isBitvector()) {
    return new NodeRef(new BitvectorOp(op, node, other->node));
  } else {
    throw PyILAException(PyExc_TypeError, "Incorrect type for " +
...
NodeRef* NodeRef::_binOp(BitvectorOp::Op op, int r) const {
  if (node->type.isBitvector()) {
    nptr_t node_r(new BitvectorConst(r, node->type.bitWidth));

    return new NodeRef(new BitvectorOp(op, node, node_r));
  } else {
    throw PyILAException(PyExc_TypeError, "Incorrect type for " +
                                              BitvectorOp::operatorNames[op]);
  }
...
NodeRef* NodeRef::_binOpR(BitvectorOp::Op op, int r) const {
  if (node->type.isBitvector()) {
    nptr_t node_l(new BitvectorConst(r, node->type.bitWidth));

    return new NodeRef(new BitvectorOp(op, node_l, node));
  } else {
    throw PyILAException(PyExc_TypeError, "Incorrect type for " +
                                              BitvectorOp::operatorNames[op]);
  }
...
NodeRef* NodeRef::_cmpOp(BoolOp::Op op, NodeRef& other, bool bvtype) const {
  if (node->type == other.node->type && (!bvtype || node->type.isBitvector())) {
    return new NodeRef(new BoolOp(op, node, other.node));
  } else {
    throw PyILAException(PyExc_TypeError,
                         "Incorrect type for " + BoolOp::operatorNames[op]);
    return NULL;
  }
}
...
// bvtype = true means this operator can only be applied
// on bitvector type objects.
NodeRef* NodeRef::_cmpOp(BoolOp::Op op, NodeRef& l, NodeRef& r, bool bvonly) {
  if (l.node->type == r.node->type && (!bvonly || l.node->type.isBitvector())) {
    return new NodeRef(new BoolOp(op, l.node, r.node));
  } else {
    throw PyILAException(PyExc_TypeError,
                         "Incorrect type for " + BoolOp::operatorNames[op]);
    return NULL;
...
NodeRef* NodeRef::_binOp(BoolOp::Op boolOp, BitvectorOp::Op bvOp,
  ...
  if (boolOp != BoolOp::INVALID && l->node->type.isBool()) {
    return new NodeRef(new BoolOp(boolOp, l->node, r->node));
  } else if (bvOp != BitvectorOp::INVALID && l->node->type.isBitvector()) {
    return new NodeRef(new BitvectorOp(bvOp, l->node, r->node));
  } else {
    throw PyILAException(PyExc_TypeError,
                         std::string("Incorrect type for ") + opName);
    return NULL;
...
NodeRef* NodeRef::_binOp(BitvectorOp::Op op, NodeRef* l, int r) {
  if (l->node->type.isBitvector()) {
    if (op >= BitvectorOp::Op::LROTATE && op <= BitvectorOp::Op::S_EXT) {
...
NodeRef* NodeRef::_triOp(BoolOp::Op boolOp, BitvectorOp::Op bvOp,
                         MemOp::Op memOp, NodeRef& cond, NodeRef& trueExp,
                         NodeRef& falseExp) {
  if (trueExp.node->type != falseExp.node->type || !cond.node->type.isBool()) {
    ...
    if (trueExp.node->type.isBool() && boolOp != BoolOp::INVALID) {
      return new NodeRef(new BoolOp(boolOp, args_));
    } else if (trueExp.node->type.isBitvector() &&
               bvOp != BitvectorOp::INVALID) {
      return new NodeRef(new BitvectorOp(bvOp, args_));
...
NodeRef* NodeRef::_naryOp(BitvectorOp::Op bvOp, nptr_vec_t& args) {
  ...
  if (!args[0]->type.isFunc()) {
    throw PyILAException(PyExc_TypeError, "Incorrect type for apply function.");
    return NULL;
  } else {
...
NodeRef* NodeRef::_choice(const std::string& name, const nptr_vec_t& args) {

  NodeType t = Choice::getChoiceType(args);
  if (!t) {
    throw PyILAException(PyExc_TypeError, "Type mismatch in choice arguments.");
    return NULL;
  } else {
    if (t.isBool()) {
