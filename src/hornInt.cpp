...
void HornTranslator::initBoolOpInt(const BoolOp* n, hvptr_t v) {
  ...
  if (n->op == BoolOp::Op::NOT) {
    ...
    if ((n->op >= BoolOp::Op::AND && n->op <= BoolOp::Op::XOR) ||
        (n->op >= BoolOp::Op::IMPLY && n->op <= BoolOp::Op::EQUAL)) {
...
void HornTranslator::initBvOpInt(const BitvectorOp* n, hvptr_t v) {
  ...
  if (n->op >= BitvectorOp::Op::NEGATE && n->op <= BitvectorOp::Op::EXTRACT) {
    ...
    if (n->op == BitvectorOp::Op::NEGATE) {
      ...
      boost::format cplFmt("(= %1% (bv2int (bvnot ((_ int2bv %2%) %3%))))");
      ...
      if (isLongBv(n->type.bitWidth)) {
        ...
        // (= (select z 0) x)
        info("Horn-Warning")
            << "Long bitvector zero extend " << v->getName() << ".\n";
      ...
      if (isLongBv(n->arg(0)->type.bitWidth)) {
        // XXX Wrong implementation.
        info("Horn-Warning")
            << "Long bitvector extract " << v->getName() << ".\n";
        ...
        extractLongBv % v->getName() % arg0->getName() % (n->param(1) / 8);
      ...
      extractOp % v->getName() % n->param(0) % n->param(1) %
          n->arg(0)->type.bitWidth % arg0->getName();
    ...
    if (isLongBv(n->type.bitWidth)) {
      // XXX Wrong implementation.
      // (= z x)
      info("Horn-Warning") << "Long bitvector binary operation " << v->getName()
                           << ".\n";
    ...
    if (n->op == BitvectorOp::Op::ADD || n->op == BitvectorOp::Op::SUB ||
        (n->op >= BitvectorOp::Op::SDIV && n->op <= BitvectorOp::Op::SMOD) ||
        n->op == BitvectorOp::Op::MUL || n->op == BitvectorOp::Op::READMEM) {
      // (= z (op x y))
      boost::format binFmt("(= %1% (%2% %3% %4%))");
      binFmt % v->getName() % bvOpNames[n->op] % arg0->getName() %
          arg1->getName();
      ...
      binFmt % v->getName() % bvOpNames[n->op] % n->arg(0)->type.bitWidth %
          arg0->getName() % n->arg(1)->type.bitWidth % arg1->getName();
      v->setExec(binFmt.str());
    } else if (n->op == BitvectorOp::Op::GET_BIT) {
      // (= z (bv2int ((_ extract 0 0) (bvlshr ((_ int2bv w) x) ((_ int2bv w)
      // y)))))
      boost::format getbitFmt("(= %1% (bv2int ((_ extract 0 0) (bvlshr ((_ "
                              "int2bv %2%) %3%) ((_ int2bv %4%) %5%)))))");
      getbitFmt % v->getName() % n->arg(0)->type.bitWidth % arg0->getName() %
          n->arg(1)->type.bitWidth % arg1->getName();
    ...
    if (!detail || (chunkNum == 1)) {
      ...
      singleReadFmt % v->getName() % mem->getName() % addr->getName();
    ...
    if (useArr && isLittle) {
      ...
      boost::format partEqFmt("(= (select %1% %2%) (select %3% (+ %4% %5%)))");
      for (int i = 0; i < chunkNum; i++) {
        partEqFmt % v->getName() % i % mem->getName() % addr->getName() % i;
      ...
      boost::format partEqFmt("(= (select %1% %2%) (select %3% (+ %4% %5%)))");
      for (int i = 0; i < chunkNum; i++) {
        partEqFmt % v->getName() % i % mem->getName() % addr->getName() %
            (chunkNum - 1 - i);
        v->setExec(partEqFmt.str());
      }
    } else if (!useArr && isLittle) {
      ...
      boost::format eqFmt("   %1%)");

      headFmt % chunkSize % mem->getName() % addr->getName();
      v->setExec(headFmt.str());

      for (int i = 1; i < chunkNum - 1; i++) {
        bodyFmt % chunkSize % mem->getName() % addr->getName() % i;
        v->setExec(bodyFmt.str());
      }

      if (chunkNum > 1) {
        tailFmt % chunkSize % mem->getName() % addr->getName() % (chunkNum - 1);
      ...
      headFmt % chunkSize % mem->getName() % addr->getName() % (chunkNum - 1);
      v->setExec(headFmt.str());
...
void HornTranslator::initMemOpInt(const MemOp* n, hvptr_t v) {
  ...
  if (n->op == MemOp::Op::STORE) {
    ...
    storeFmt % v->getName() % arg0->getName() % arg1->getName() %
        arg2->getName();
    ...
    if (_iteAsNode) {
      ...
      iteFmt % v->getName() % arg0->getName() % arg1->getName() %
          arg2->getName();
      v->setExec(iteFmt.str());
    } else {
      v->setExec("true");
    ...
    if (!detail || (chunkNum == 1)) {
      ...
      singleWriteFmt % v->getName() % arg0->getName() % arg1->getName();
    ...
    boost::format arrEqFmt("(= %1% %2%)");
    if (useArr && isLittle) {
      // (= z_2 (store A   (+ addr 2) (select val 2)))
      // (= z_1 (store z_2 (+ addr 1) (select val 1)))
      // (= z_0 (store z_1 (+ addr 0) (select val 0)))
      // (= z z_0)
      arrStoreFmt % addSuffix(v->getName(), chunkNum - 1) % arg0->getName() %
          arg1->getName() % (chunkNum - 1) % arg2->getName() % (chunkNum - 1);
    ...
    if (useArr && !isLittle) {
      ...
      // (= z z_0)
      arrStoreFmt % addSuffix(v->getName(), chunkNum - 1) % arg0->getName() %
          arg1->getName() % 0 % arg2->getName() % (chunkNum - 1);
      v->setExec(arrStoreFmt.str());

      for (int i = chunkNum - 2; i >= 0; i--) {
        arrStoreFmt % addSuffix(v->getName(), i) %
            addSuffix(v->getName(), i + 1) % arg1->getName() %
            (chunkNum - 1 - i) % arg2->getName() % i;
        v->setExec(arrStoreFmt.str());
      }
    ...
    boost::format memBlkAddFmt("(= %1% (store %2% (+ %3% %4%) (bv2int ((_ "
                               "extract %5% %6%) ((_ int2bv %7%) %8%)))))");
    boost::format memBlkOrgFmt("(= %1% (store %2% %3% (bv2int ((_ extract %4% "
                               "%5%) ((_ int2bv %6%) %7%)))))");

    if (isLittle) {
      // FIXME
      // (= z_2 (store A      addr    (bv2int ((_ extract 7 0)   ((_ int2bv w)
      // val)))))
      // (= z_1 (store z_2 (+ addr 1) (bv2int ((_ extract 15 8)  ((_ int2bv w)
      // val)))))
      // (= z   (store z_1 (+ addr 2) (bv2int ((_ extract 23 16) ((_ int2bv w)
      // val)))))
      memBlkOrgFmt % addSuffix(v->getName(), chunkNum - 1) % arg0->getName() %
          arg1->getName() % (chunkSize - 1) % 0 % dataSize % arg2->getName();
      ...
      for (int i = 1; i < chunkNum - 1; i++) {
        memBlkAddFmt % addSuffix(v->getName(), i) %
            ...
            (chunkSize * (chunkNum - i - 1)) % dataSize % arg2->getName();
        v->setExec(memBlkAddFmt.str());
      }

      memBlkAddFmt % v->getName() % addSuffix(v->getName(), 1) %
          arg1->getName() % (chunkNum - 1) % (chunkSize * chunkNum - 1) %
          (chunkSize * (chunkNum - 1)) % dataSize % arg2->getName();
      v->setExec(memBlkAddFmt.str());
    } else {
      // FIXME
      // (= z_2 (store A   (+ addr 2) (bv2int ((_ extract 7 0)   ((_ int2bv w)
      // val)))))
      // (= z_1 (store z_2 (+ addr 1) (bv2int ((_ extract 15 8)  ((_ int2bv w)
      // val)))))
      // (= z   (store z_1    addr    (bv2int ((_ extract 23 16) ((_ int2bv w)
      // val)))))
      memBlkAddFmt % addSuffix(v->getName(), chunkNum - 1) % arg0->getName() %
          arg1->getName() % (chunkNum - 1) % (chunkSize - 1) % 0 % dataSize %
          arg2->getName();
      v->setExec(memBlkAddFmt.str());
      ...
      memBlkOrgFmt % v->getName() % addSuffix(v->getName(), 1) %
