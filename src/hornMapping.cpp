...
void HornRewriter::update(const std::string& state, hvptr_t inArg,
                          hvptr_t outArg) {
  if (inArg != NULL)
    _mapSI[state] = inArg;
  if (outArg != NULL)
...
std::string HornRewriter::rewrite(char inType, int inSuff, char outType,
                                  int outSuff) {
  if (_pred->getInNum() == 0) {
  ...
  for (auto itIn = inSet.begin(); itIn != inSet.end(); itIn++) {
    ...
    inArgs += " ";
    inArgs +=
        (inSuff == -1) ? var->getName() : _tr->copyVar(var, inSuff)->getName();
  }

  std::string outArgs = "";
  for (auto outVar : outSet) {
    hvptr_t var =
        (outType == 'I') ? _mapSI[_mapOS[outVar]] : _mapSO[_mapOS[outVar]];
    outArgs += " ";
...
void HornRewriter::addRewriteRule(hcptr_t C, char aType, int aSuff, char bType,
                                  int bSuff) {
  boost::format eqFmt("(= %1% %2%)");
  auto outSet = _pred->getOutSet();
  for (auto outVar : outSet) {
    hvptr_t varA =
        (aType == 'I') ? _mapSI[_mapOS[outVar]] : _mapSO[_mapOS[outVar]];
    hvptr_t varB =
        (bType == 'I') ? _mapSI[_mapOS[outVar]] : _mapSO[_mapOS[outVar]];
...
hvptr_t HornRewriter::getRewriteVar(char inTy, int inSu, char outTy,
                                    int outSu) {
...
void HornTranslator::addChildInstr(const std::string& c, const std::string& i,
                                   NodeRef* d) {
  ...
  for (auto itI = _instrs.begin(); itI != _instrs.end(); itI++) {
    ...
    for (auto uName : instr->_childInstrs) {
      ...
      for (auto itUN = uInstr->_nxtFuncs.begin();
        ...
        loopRW->update(itUN->first, NULL, itUN->second->getOutVar());
      }
    ...
    for (auto itN = instr->_nxtFuncs.begin(); itN != instr->_nxtFuncs.end();
         itN++) {
      auto nxt = itN->second;
      ...
      mRW->update(itN->first, NULL, itN->second->getOutVar());

      loopRW->update(itN->first, NULL, itN->second->getOutVar());
    }
  ...
  for (auto itI = _instrs.begin(); itI != _instrs.end(); itI++) {
    ...
    for (auto uName : instr->_childInstrs) {
      ...
      for (auto itUN = uInstr->_nxtFuncs.begin();
        ...
        cRW->update(itUN->first, NULL, itUN->second->getOutVar());
      }
...
HornRewriter* HornTranslator::generateLoopPredicate() {
  ...
  for (auto itU = _childs.begin(); itU != _childs.end(); itU++) {
    ...
    lVar->mergeInVars(uInstr->_decodeFunc);
    for (auto itUN = uInstr->_nxtFuncs.begin(); itUN != uInstr->_nxtFuncs.end();
         itUN++) {
      auto uNxt = itUN->second;
