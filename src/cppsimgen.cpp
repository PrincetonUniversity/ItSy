...
CppVar::CppVar(const std::string& name, const std::string& val) : _val(val) {
...
std::string CppVar::refDef() const { return (vType() + "& " + _name); }
...
// ---------------------------------------------------------------------- //
void CppVar::init(nptr_t n) { init(n.get()); }
...
// ---------------------------------------------------------------------- //
CppFun::CppFun(const std::string& name) : _name(name) { _ret = NULL; }

CppFun::~CppFun() {}

// ---------------------------------------------------------------------- //
void CppFun::addArg(const CppVar* arg) { _args.push_back(arg); }

void CppFun::addBody(const std::string& code) { _codeList.push_back(code); }

// ---------------------------------------------------------------------- //
// Print the function header outside the class to the output stream.
void CppFun::dumpDec(std::ostream& out, const std::string& modelName,
  ...
  std::string name = (modelName == "") ? _name : (modelName + "::" + _name);
...
void CppSimGen::endFun(CppFun* f) {
  ...
  for (unsigned i = 0; i < f->_updates.size(); i++) {
    code = f->_updates[i].first->use() + " = " + f->_updates[i].second->use() +
           ";";
    f->addBody(code);
  }
  // Return the value.
...
void CppSimGen::createCommon(std::ostream& out) const {
  ...
  out << "typedef boost::multiprecision::cpp_int " << CppVar::bvStr << ";\n";
  out << "#endif\n";
...
CppVar* CppSimGen::getBvOpCpp(const BitvectorOp* n) {
  ...
  if (n->op >= BitvectorOp::Op::NEGATE && n->op <= BitvectorOp::Op::EXTRACT) {
    ...
    if (n->op == BitvectorOp::Op::ADD) {
      ...
      code = var->use() + " = " + arg0->use() + ".rd(" + arg1->use() + ");";
    }
    //// Ternary ////
    ...
    boost::format defFmt("%1% = (%2%.rd(%3%) & %4%);");
    boost::format loopProlog("for (int %1% = 1; %2% < %3%; %4%++) {");
    boost::format litFmt(
        "\t%1% = %2% | ((%3%.rd(%4% + %5%) & %6%) << (%7% * %8%));");
    boost::format bigFmt("\t%1% = (%2% << %3%) | (%4%.rd(%5% + %6%) & %7%);");
...
CppVar* CppSimGen::getMemOpCpp(const MemOp* n) {
  ...
  if (n->op == MemOp::Op::STORE) {
    ILA_ASSERT(arg0->_type == CppVar::memStr, "Write to non-mem variable.");
    var = new CppVar(n);
    code = var->use() + " = " + arg0->use() + ";";
    _curFun->addBody(code);
    code = var->use() + ".wr(" + arg1->use() + ", " + arg2->use() + ");";
  } else if (n->op == MemOp::Op::ITE) {
    ILA_ASSERT(arg0->_type == CppVar::boolStr, "Condition not bool.");
    var = new CppVar(n);
    code = var->refDef() + " = (" + arg0->use() + ") ? " + arg1->use() + " : " +
           arg2->use() + ";";
    ...
    boost::format writeFmt("\t%1%.wr(%2% + %3%, %4%);");
...
void CppSimGen::defMemClass(std::ostream& out) const {
  ...
  out << "\t{\n";
  out << "\t\tstd::map<" << bvStr << ", " << bvStr
      << ">::iterator it = _map.find(addr);\n";
  out << "\t\tif (it == _map.end()) {\n";
  out << "\t\t\treturn _def_val;\n";
  out << "\t\t} else {\n";
...
void CppSimGen::setMemConst(std::ostream& out) const {
  ...
  for (auto it = _memConst.begin(); it != _memConst.end(); it++) {
    ...
    for (auto p : it->second->memvalues.values) {
      out << it->first->use() << ".wr(" << p.first << ", " << p.second
          << ");\n";
    }
  }
  out << "/****************************************************/\n";
...
void CppSimGen::genModel(std::ostream& out) const {
  ...
  for (auto it = _masks.begin(); it != _masks.end(); it++) {
    out << "\t\t" << it->second->use() << " = " << it->second->_val << ";\n";
  }
  out << "\t};\n"
      << "\t~" << _modelName << "() {};\n";
