...
void VerilogExport::exportMem(const std::string& name, const npair_t& np) {
  ...
  ILA_ASSERT((memvar = dynamic_cast<const MemVar*>(np.var.get())),
             "memory variable type mismatched");
...
void VerilogExport::exportCondWrites(const std::string& name, int addrWidth,
                                     int dataWidth,
...
void VerilogExport::exportFunc(const std::string& name, const npair_t& np) {
  ...
  ILA_ASSERT((funcvar = dynamic_cast<const FuncVar*>(np.var.get())),
             "function variable type mismatched.");

  if (FunctionAsModule) {
    vlg_stmt_t funcModDef = "module fun_" + funcvar->getName() + " (\n";
    int argNo = 1;
    for (const auto& argWidth : funcvar->type.argsWidth)
      funcModDef += "    input " + WidthToRange(argWidth) + " arg" +
                    toStr(argNo++) + ",\n";
    funcModDef +=
        "    output " + WidthToRange(funcvar->type.bitWidth) + " result\n";
    funcModDef += ");\n";
    ...
    for (const auto& argWidth : funcvar->type.argsWidth)
      funcDecal += vlg_stmt_t("input ") + WidthToRange(argWidth) + " arg" +
                   toStr(argNo++) + ";\n";
...
void VerilogExport::add_stmt(const vlg_stmt_t& s) { statements.push_back(s); }
...
void VerilogExport::add_ite_stmt(const vlg_stmt_t& cond,
                                 const vlg_stmt_t& tstmt,
                                 const vlg_stmt_t& fstmt) {
  ite_stmts.push_back(std::make_tuple(cond, tstmt, fstmt));
}

void VerilogExport::add_mem(const vlg_name_t& n, int addr_width,
                            int data_width) {
...
vlg_name_t VerilogExport::translateBoolOp(const BoolOp* boolop) {
  ...
  ILA_ASSERT(op != BoolOp::INVALID,
             "Cannot translate invalid operator to Verilog.");
  ...
  vlg_stmt_t tmp_stmt =
      vlg_stmt_t("assign ") + tmp_result + " = " + result_stmt + " ;";
...
vlg_name_t VerilogExport::translateBitvectorOp(const BitvectorOp* bvop) {
  ...
  if (op == BitvectorOp::NEGATE) { // negate : 2's complement
    ...
    if (rotN == 0)
      ...
      result_stmt = vlg_stmt_t("  { ") + arg0 + " [" + toStr(width - rotN) +
                    ":0] ," + arg0 + "[" + toStr(width) + ":" +
                    toStr(width - rotN + 1) + "] }";
    }
    ...
    if (outWidth == bvWidth)
      ...
      result_stmt =
          vlg_stmt_t(" {") + toStr(outWidth - bvWidth) + "'d0 , " + arg0 + "} ";
    }
    ...
    if (outWidth == bvWidth)
      ...
      result_stmt = vlg_stmt_t(" { {") + toStr(outWidth - bvWidth) + "{" +
                    arg0 + "[" + toStr(bvWidth - 1) + "] }  }, " + arg0 + "} ";
    }
    ...
    if (op == BitvectorOp::ADD) {
      ...
      if (ExternalMem) {
        ...
        vlg_stmt_t tmp_stmt =
            vlg_stmt_t("assign ") + addrName + " = " + arg2 + " ;";
        add_stmt(tmp_stmt);
      ...
      for (int i = 0; i < chunks; i++) {
        ...
        if (ExternalMem) {
          ...
          vlg_stmt_t tmp_stmt = vlg_stmt_t("assign ") + addrName + " = " +
                                arg2 + "+" + toStr(i) + " ;";
          add_stmt(tmp_stmt);
      ...
      log1("VerilogExport")
          << BitvectorOp::operatorNames[op] << " not supported.\n";
      ILA_ASSERT(false, "Operator not supported.");
    }
    ...
    if (FunctionAsModule) {
      ...
      vlg_stmt_t funcInstantiation =
          "fun_" + funcVar->getName() + "  " + "applyFunc_" + NewId() + "(\n";
      int arity = bvop->nArgs();
      for (int i = 1; i != arity; i++) {
        funcInstantiation +=
            "    .arg" + toStr(i) + "( " + getArg(bvop, i) + " ),\n";
      }
    ...
    log1("VerilogExport") << BitvectorOp::operatorNames[op]
                          << " not supported.\n";
    ILA_ASSERT(false, "Operator not supported.");
  }
...
void VerilogExport::visitMemNodes(const Node* n, const nptr_t& cond,
  ...
  ILA_ASSERT(memvar != NULL || memop != NULL, "expected a memvar or memop.");
...
vlg_name_t VerilogExport::NewId() { return "n" + toStr(idCounter++); }
...
VerilogExport::VerilogExport(const std::string& modName, const std::string& clk,
    ...
    : moduleName(modName), clkName(clk), rstName(rst),
      ...
      notCache(NUM_HASHTABLE_BUCKETS_SMALL, nodeHash, nodeEqual), idCounter(0),
      ExternalMem(config._extMem), FunctionAsModule(config._fmodule) {}
...
void VerilogExport::finalExport(std::ostream& fout) {
  ...
  for (auto const& sig_pair : regs)
    fout << "reg " << std::setw(10) << WidthToRange(sig_pair.second) << " "
         << (sig_pair.first) << ";\n";
  ...
  if (ExternalMem) {
    ...
    for (auto const& mem : mems)
      fout << "reg " << std::setw(10) << WidthToRange(std::get<2>(mem)) << " "
           << (std::get<0>(mem)) << WidthToRange(std::pow(2, std::get<1>(mem)))
           << ";\n";
  }
