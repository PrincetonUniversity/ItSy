...
CVar::CVar(const CVar* var)
    : _type(var->_type), _width(var->_width), _idxWidth(var->_idxWidth),
...
std::string CVar::refDef() const { return (_type + "& " + _name); }
...
std::string CVar::signedUse() const {
  boost::format signUseFmt("((%1% & %2%) ? (%3% | %4%) : %5%)");
  if (_isConst) {
    return _val;
  } else {
    signUseFmt % _name % getSignBit(_width) % _name % getSignExtMask(_width) %
        _name;
...
// ---------------------------------------------------------------------- //
void CVar::init(nptr_t n) { init(n.get()); }
...
// ---------------------------------------------------------------------- //
CFun::CFun(const std::string& name) : _name(name) { _ret = NULL; }

CFun::~CFun() {}

// ---------------------------------------------------------------------- //
void CFun::addArg(const CVar* arg) { _args.push_back(arg); }

void CFun::addBody(const std::string& code) { _codeList.push_back(code); }

// ---------------------------------------------------------------------- //
// Print the function header outside the class to the output stream. (also
// uninterpreted functions)
void CFun::dumpDec(std::ostream& out, const std::string& modelName,
...
CFun* CVerifGen::addFun(const std::string& name, bool ms) {
  ...
  for (auto it = _unitpFuncVarMap.begin(); it != _unitpFuncVarMap.end(); it++) {
    checkAndInsert(*varMap, it->first, it->second, ms);
  }
  ...
  for (auto it = _curVarMap->begin(); it != _curVarMap->end(); it++) {
    if (_states.find(it->first) == _states.end() &&
      ...
      if (it->second->_type == CVar::memStr) {
        code = it->second->def() + " = " + it->second->memdef() +
               ";"; // define of variables, for memory need to take care of
        _localArray[it->first] =
            it->second; // record and later may need to free it if it is not the
                        // return variable
      } else
        code = it->second->def() + ";"; // define of variables
      // don't insert the same if you export to a single file (in a single
      // update fun)
      if (std::find(_curFun->_varList.begin(), _curFun->_varList.end(), code) ==
          _curFun->_varList.end())
...
CVar* CVerifGen::appFun(CFun* appFun, CFun* envFun) {
  ...
  std::string code = retVar->def() + " = " + appFun->_name +
                     "( this"; // FIXME: need to add its struct's name
...
void CVerifGen::createCommon(std::ostream& out) const {
  ...
  out << "typedef uint64_t " << CVar::bvStr << ";\n";
  out << "#endif\n";
...
CVar* CVerifGen::getBoolOpC(const BoolOp* n) {
  ...
  if (n->op == BoolOp::Op::NOT) {
    ...
    if (n->op == BoolOp::Op::AND) {
      code = var->use() + " = " + arg0->use() + " && " + arg1->use() + ";";
...
CVar* CVerifGen::getBvOpC(const BitvectorOp* n) {
  ...
  if (n->op >= BitvectorOp::Op::NEGATE && n->op <= BitvectorOp::Op::EXTRACT) {
    ...
    if (n->op == BitvectorOp::Op::NEGATE) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " NEGATE "
                  << std::endl;
      code = var->use() + " = -" + arg0->signedUse() + ";";
    } else if (n->op == BitvectorOp::Op::COMPLEMENT) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " COMPLEMENT "
                  << std::endl;
      code = var->use() + " = ~" + arg0->use() + ";";
    } else if (n->op == BitvectorOp::Op::LROTATE) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " LROTATE "
                  << std::endl;
      int par0 = n->param(0);
      ...
      _curFun->addBody(code);
      code = var->use() + " = ((" + var->use() + " << " + l + ") | ( " +
             var->use() + " >> " + r + ")) & " + getMask(var->_width) + ";";
    } else if (n->op == BitvectorOp::Op::RROTATE) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " RROTATE "
                  << std::endl;
      int par0 = n->param(0);
      int total = var->_width;
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " Z_EXT(*) "
                  << std::endl;
      if (n->type.bitWidth > 64)
        std::cerr << "W:" << (n->type.bitWidth) << " * Z_EXT " << std::endl;
      code = var->use() + " = " + arg0->exactUse() + ";";
    } else if (n->op == BitvectorOp::Op::S_EXT) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " S_EXT(*) "
                  << std::endl;
      if (n->type.bitWidth > 64)
        std::cerr << "W:" << (n->type.bitWidth) << " * S_EXT " << std::endl;
      code = var->use() + " = " + arg0->signedUse() + ";";
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " EXTRACT(*) to "
                  << (n->type.bitWidth) << " " << msb << ":" << lsb
                  << std::endl;
      if (n->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " EXTRACT(*) to "
                  << (n->type.bitWidth) << " " << msb << ":" << lsb
    ...
    if (n->op == BitvectorOp::Op::ADD) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " ADD(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " ADD(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " + " + arg1->use() + ";";
      _curFun->addBody(code);
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " SUB(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " SUB(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " - " + arg1->use() + ";";
      _curFun->addBody(code);
      code = var->use() + " = " + var->exactUse() + ";";
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " AND(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " AND(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " & " + arg1->use() + ";";
    } else if (n->op == BitvectorOp::Op::OR) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " OR(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " OR(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " | " + arg1->use() + ";";
    } else if (n->op == BitvectorOp::Op::XOR) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " XOR(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " XOR(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " ^ " + arg1->use() + ";";
    } else if (n->op == BitvectorOp::Op::XNOR) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " XNOR(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " XNOR(R) "
                  << std::endl;
      code = var->use() + " = ~(" + arg0->use() + " ^ " + arg1->use() + ");";
    } else if (n->op == BitvectorOp::Op::NAND) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " NAND(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " NAND(R) "
                  << std::endl;
      code = var->use() + " = ~(" + arg0->use() + " & " + arg1->use() + ");";
    } else if (n->op == BitvectorOp::Op::NOR) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " NOR(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " NOR(R) "
                  << std::endl;
      code = var->use() + " = ~(" + arg0->use() + " | " + arg1->use() + ");";
    } else if (n->op == BitvectorOp::Op::SDIV) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " SDIV(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " SDIV(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->signedUse() + " / " +
             arg1->signedUse() + ";";
      _curFun->addBody(code);
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " UDIV(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " UDIV(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " / " + arg1->use() + ";";
      _curFun->addBody(code);
      code = var->use() + " = " + var->exactUse() + ";";
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " SREM(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " SREM(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " % " + arg1->use() + ";";
      _curFun->addBody(code);
      code = var->use() + " = " + var->exactUse() + ";";
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " UREM(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " UREM(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " % " + arg1->use() + ";";
      _curFun->addBody(code);
      code = var->use() + " = " + var->exactUse() + ";";
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " SMOD(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " SMOD(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " % " + arg1->use() + ";";
      _curFun->addBody(code);
      code = var->use() + " = " + var->exactUse() + ";";
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " SHL(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " SHL(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " << " + arg1->castUse() + ";";
      _curFun->addBody(code);
      code = var->use() + " = " + var->exactUse() + ";";
    } else if (n->op == BitvectorOp::Op::LSHR) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " LSHR(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " LSHR(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " >> " + arg1->castUse() + ";";
    } else if (n->op == BitvectorOp::Op::ASHR) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " ASHR(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " ASHR(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->signedUse() + " >> " + arg1->castUse() +
             ";";
      _curFun->addBody(code);
      code = var->use() + " = " + var->exactUse() + ";";
    } else if (n->op == BitvectorOp::Op::MUL) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " MUL(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " MUL(R) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + " * " + arg1->use() + ";";
      _curFun->addBody(code);
      code = var->use() + " = " + var->exactUse() + ";";
      ...
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " CONCAT(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " CONCAT(R) "
                  << std::endl;
      if (n->arg(0)->type.bitWidth + n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:"
                  << (n->arg(0)->type.bitWidth + n->arg(1)->type.bitWidth)
                  << "* CONCAT " << std::endl;
      std::string w = boost::lexical_cast<std::string>(arg1->_width);
      // |arg0| << arg1.width | |arg1|
      code = var->use() + " = (" + arg0->use() + " << " + w + ") | " +
             arg1->use() + ";";
      _curFun->addBody(code);
      code = var->use() + " = " + var->exactUse() + ";";
    } else if (n->op == BitvectorOp::Op::GET_BIT) {
      if (n->arg(0)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " GET_BIT(L) "
                  << std::endl;
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " GET_BIT(R) "
                  << std::endl;
      code = var->use() + " = (" + arg0->use() + " >> " + arg1->castUse() +
             ") & 0x1;";
    } else if (n->op == BitvectorOp::Op::READMEM) {
      if (n->arg(1)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " READMEM(addr) "
                  << std::endl;
      code = var->use() + " = " + arg0->use() + "[" + arg1->use() + "];";
    }
    //// Ternary ////
    ...
    if (n->arg(1)->type.bitWidth > 64)
      std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " READMEMBLOCK(addr) "
                << std::endl;
    ...
    boost::format defFmt("%1% = (%2%[%3%] & %4%);");
    boost::format loopProlog("for (int %1% = 1; %2% < %3%; %4%++) {");
    boost::format litFmt(
        "\t%1% = %2% | ((%3%[%4% + %5%] & %6%) << (%7% * %8%));");
    boost::format bigFmt("\t%1% = (%2% << %3%) | (%4%[%5% + %6%] & %7%);");
    ...
    CVar* arg2 = findVar(*_curVarMap, n->arg(2)->getName());
    if (n->arg(1)->type.bitWidth > 64)
      std::cerr << "W:" << (n->arg(1)->type.bitWidth) << " IF(L) " << std::endl;
    if (n->arg(2)->type.bitWidth > 64)
      std::cerr << "W:" << (n->arg(2)->type.bitWidth) << " IF(R) " << std::endl;
    code = var->use() + " = (" + arg0->use() + ") ? " + arg1->use() + " : " +
           arg2->use() + ";";
    ...
    for (unsigned i = 1; i != n->nArgs(); i++) {
      ...
      if (n->arg(i)->type.bitWidth > 64)
        std::cerr << "W:" << (n->arg(0)->type.bitWidth) << " APPLY_FUNC(" << i
                  << ") " << std::endl;
...
CVar* CVerifGen::getMemOpC(const MemOp* n) // FIXME: need to change
{
  ...
  if (n->op == MemOp::Op::STORE) {
    ILA_ASSERT(arg0->_type == CVar::memStr, "Write to non-mem variable.");
    var = new CVar(n);
    ...
    _curFun->addBody(code);
    code = var->use() + "[" + arg1->use() + "] = ( " + arg2->use() + " );";
  } else if (n->op == MemOp::Op::ITE) {
    ILA_ASSERT(arg0->_type == CVar::boolStr, "Condition not bool.");
    var = new CVar(n);
    code = var->use() + " = " + var->memdef() + ";";
    _curFun->addBody(code);

    boost::format memite("if(%1%) \n\t__CPROVER_array_copy( %2% ,  %3%); "
                         "\nelse\n\t__CPROVER_array_copy( %2% ,  %4%);");

    memite % arg0->use() % var->use() % arg1->use() % arg2->use();
    ...
    ILA_ASSERT(arg2->_width % chunkSize == 0, "Block store size mismatch.");
    ...
    boost::format loopProlog("for (int %1% = 0; %2% < %3%; %4%++) {");
    boost::format litFmt("\t%1% = %2% >> (%3% * %4%);");
    boost::format bigFmt("\t%1% = %2% >> ((%3% - 1 - %4%) * %5%);");
    boost::format uniFmt("\t%1% = %2%;");
    boost::format writeFmt("\t%1%[%2% + %3%] =(%4%);");
...
void CVerifGen::setMemConst(std::ostream& out) const {
  ...
  for (auto it = _memConst.begin(); it != _memConst.end(); it++) {
    ...
    for (auto p : it->second->memvalues.values) {
      out << it->first->use() << "[" << p.first << "] = ( " << p.second
          << ");\n";
    }
  }
  out << "/****************************************************/\n";
...
void CVerifGen::genModel(std::ostream& out) const {
  ...
  out << "void " << _modelName << "_init( struct " << _modelName
      << " * this ) {\n";
  for (auto it = _states.begin(); it != _states.end(); it++) {
    if (it->second->_type == CVar::memStr)
  ...
  out << "" << _modelName << "_destroy( struct " << _modelName
      << " * this ) {\n";
  for (auto it = _states.begin(); it != _states.end(); it++) {
    if (it->second->_type == CVar::memStr)
  ...
  for (auto it = _funMap.begin(); it != _funMap.end(); it++) {
    it->second->dumpDec(out, _modelName, 0,
                        true); // even we give modelname, we still want it to be
...
std::string CVerifGen::getSignedCCode(CVar* var) {
  ...
  boost::format signUseFmt("%1% = ((%2% >> %3%) & 0x1) ? %4% : %5%;");
  signUseFmt % var->use() % var->use() % (var->_width - 1) %
      signTempVar->use() % unsignTempVar->use();
  return signUseFmt.str();
}
...
void CVerifGen::checkAndInsert(std::map<std::string, T*>& mp,
                               const std::string& name, T* var, bool force) {
