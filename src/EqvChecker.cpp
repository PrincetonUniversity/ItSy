...
bool NEQVarNotExist(const std::string& a1name, const std::string& a2name,
  ...
  std::cout << "State " << notExistVarName << " of " << a1name
            << " does not exist in " << a2name << std::endl;
  return false;
}
bool NEQVarTypeMismatch(const std::string& a1name, const std::string& a2name,
                        const std::string& VarName) {
  std::cout << "State " << VarName << " of " << a1name
            << " does not match the one in " << a2name << std::endl;
  return false;
}
bool NEQArchVarUpdateMismatch(const std::string& a1name,
                              const std::string& a2name,
                              const std::string& VarName) {
  std::cout << "Update function of state " << VarName << " of " << a1name
            << " does not match the one in " << a2name << std::endl;
  return false;
}
...
unsigned DetermineUnrollBound(Abstraction* pAbs, const std::string& nodeName) {
  ...
  if (NoUpdate) {
    std::cout << "Note: " << nodeName << " in " << (pAbs->name)
              << " is not updated by micro/sub ILA." << std::endl;
    return 1;
  }
  return -1;
}
