...
void ITESimplifier::addAssumption(const nptr_t& a) { _add(a); }
...
void ITESimplifier::createClone(const Node* n) {
  ...
  if ((boolvar = dynamic_cast<const BoolVar*>(n))) {
    ...
    nptr_t nptr(ReadSlice::createReadSlice(rdslice->getName(), bvp,
                                           rdslice->width, rdslice->increment));
    rwmap.insert({n, nptr});
  } else if ((wrslice = dynamic_cast<const WriteSlice*>(n))) {
    nptr_t bvp = getRepl(wrslice->bitvec.get());
    nptr_t wrp = getRepl(wrslice->data.get());
    nptr_t nptr(WriteSlice::createWriteSlice(wrslice->getName(), bvp, wrp,
                                             wrslice->increment));
    rwmap.insert({n, nptr});
...
// ---------------------------------------------------------------------- //
void ITESimplifier::getNewArgs(const Node* op, nptr_vec_t& args) {
  unsigned n = op->nArgs();
...
void ITESimplifier::dfs(const Node* n) {
  ...
  if ((bvop && bvop->getOp() == BitvectorOp::IF) ||
    ...
    if (r_true == z3::sat) {
      log2("ITESimplifier.dfs")
          << "ite: TRUE sat: " << *(cond.get()) << std::endl;
      dfs(t_val.get());
    }
    S.pop();
    ...
    if (r_false == z3::sat) {
      log2("ITESimplifier.dfs")
          << "ite: FALSE sat: " << *(cond.get()) << std::endl;
      dfs(f_val.get());
    }
    S.pop();
